...
1. 刚刚从BIOS进入到了loader代码，一开始的代码是loader_16.c，做了两个工作：

   1. 内存检测，得到的结果信息储存在结构体`boot_info`中
   2. 开启保护模式

2. 进入保护模式，需要：关闭中断、开启A20地址线、加载gdt表、cr0寄存器使能、远跳转至32位的代码loader_32.c。**需要注意，开启保护模式后BIOS中断将无法使用**

3. loader_32.c的工作就是加载内核。首先执行的函数是`load_kernel(void)`，使用LBA48模式读取磁盘，把磁盘中的内核代码`kernel.elf`（在第100个扇区）写入内存中的 0x100000 (1MiB) 处。

4. 然后执行 `reload_elf_file(1024 * 1024)`函数，意思是解析0x100000处存放的elf内容。该函数将elf文件解析，放置在内存0x10000处，也就是64KiB处。

5. 这样内核就加载完了。

   此时，需要把内存检测的信息让内核代码使用。

   这里使用的做法是函数调用传参数，将内核的入口地址处视为存放着一个接收启动信息参数的函数。调用这个函数的时候即意味着跳转到这个地址，并取出了`boot_info`参数，可以使用。代码如此：`((void (*)(boot_info_t*))0x10000)(&boot_info)`。

   这就表示直接跳到 0x10000 处运行，也就是刚刚解析elf文件加载的内容